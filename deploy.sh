...
full () {
    ...
    docker create --name nginx --net=baltic --log-driver=fluentd --log-opt fluentd-async-connect=true --restart=unless-stopped -v $HOME/balticweb/nginx/conf.d:/etc/nginx/conf.d -p 443:443 -p 80:80 nginx:stable
}
